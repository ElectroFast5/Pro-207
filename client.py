            ...
            if(numberBox['text']):
                if(int(numberBox['text']) not in flashNumberList):
                    if(platform.system() == 'Darwin'):
                        numberBox.configure(state='disabled', disabledbackground='red',
                            disabledforeground="pink")
                    else:
                        numberBox.configure(state='disabled', background='red',
                            foreground="pink")
...
def markNumber(button):
    ...
    if(platform.system() == 'Darwin'):
        button.configure(state='disabled',disabledbackground='lime', disabledforeground="purple", highlightbackground="lime")
    else:
        button.configure(state='disabled',background='lime', foreground="purple")
    ...
    for row in range(0,3):
        ...
        while (counter < len(randomColList)):
            ...
            if(randomNumber not in currentNumberList):
                ...
                numberBox.configure(text=randomNumber, fg="purple")
    ...
    for row in ticketGrid:
        for numberBox in row:
            if(not numberBox['text']):
                if(platform.system() == 'Darwin'):
                    numberBox.configure(state='disabled', disabledbackground='navy', highlightbackground='navy')
                else:
                    numberBox.configure(state='disabled', background='navy')


def createTicket():
    global gameWindow
    global ticketGrid
    mianLable = Label(gameWindow, width=65, height=16,relief='ridge', borderwidth=5, bg='pink')
    ...
    for row in range(0, 3):
        ...
        for col in range(0, 9):
            if(platform.system() == 'Darwin'):
                ...
                boxButton = tk.Button(gameWindow, font = ("Chalkboard SE",30), width=3, height=2,borderwidth=5, bg="yellow")
                boxButton.configure(command = lambda boxButton=boxButton : markNumber(boxButton))
                boxButton.place(x=xPos, y=yPos)
...
def gameWindow():
    ...
    canvas2.create_text( screen_width/4.5,50, text = "Tambola Family Fun", font=("Chalkboard SE",50), fill="orange")
    createTicket()
    placeNumbers()
    flashNumberLabel = canvas2.create_text(400,screen_height/1.3, text = "Waiting for other to join...", font=("Chalkboard SE",30), fill="orange")
...
def askPlayerName():
    ...
    canvas1.create_text( screen_width/4.5,screen_height/8, text = "Enter Name", font=("Chalkboard SE",60), fill="green")
    nameEntry = Entry(nameWindow, width=15, justify='center', font=('Chalkboard SE', 30), bd=5, bg='cyan')
    nameEntry.place(x = screen_width/7, y=screen_height/5.5 )
    button = tk.Button(nameWindow, text="Save", font=("Chalkboard SE", 30),width=11, command=saveName, height=2, bd=3,bg="cyan")
...
def setup():
    ...
    thread = Thread(target=receivedMessage)
    thread.start()
